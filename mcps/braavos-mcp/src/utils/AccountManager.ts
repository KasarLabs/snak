...
export class AccountManager implements BaseUtilityClass {
  ...
  async estimateAccountDeployFee(
    ...
    try {
      ...
      throw new Error(`Failed to estimate deploy fee: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async deployAccount(
    ...
    try {
      ...
      throw new Error(`Failed to deploy account: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

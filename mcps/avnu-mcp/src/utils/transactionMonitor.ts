...
export class TransactionMonitor implements BaseUtilityClass {
  ...
  async waitForTransaction(
    ...
    while (true) {
      try {
        ...
        if (error instanceof Error && error.message.includes('Transaction hash not found')) {
          await new Promise((resolve) =>
            setTimeout(resolve, this.pollingInterval)
          );
          continue;
  ...
  async getTransactionEvents(txHash: string): Promise<Event[]> {
    try {
      ...
      throw new Error(`Failed to get transaction events: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

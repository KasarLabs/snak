...
export class Checker {
  ...
  async checkWorld(): Promise<number> {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : 'Error when check the world ID for artpeace'
      );
    }
  }
  ...
  async checkPosition(x: number, y: number): Promise<number> {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : 'Error when check the position for artpeace'
      );
    }
  }
  ...
  async checkColor(color: string): Promise<string> {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : 'Error when check the colors for artpeace'
      );
    }
  }

  ...
  public async validateAgent(
    ...
    try {
      ...
      if (agent_config.name) {
        if (
          agent_config.name.length > getGuardValue('agents.name_max_length')
        ) {
          throw new Error(
        ...
        if (
          agent_config.name.length < getGuardValue('agents.name_min_length')
        ) {
          throw new Error(
      ...
      if (agent_config.group) {
        if (
          agent_config.group.length > getGuardValue('agents.group_max_length')
        ) {
          throw new Error(
        ...
        if (
          agent_config.group.length < getGuardValue('agents.group_min_length')
        ) {
          throw new Error(
      ...
      if (agent_config.max_iterations) {
        if (
          ...
          getGuardValue('agents.max_max_iterations')
        ) {
          throw new Error(
        ...
        if (
          agent_config.max_iterations <
          getGuardValue('agents.min_max_iterations')
        ) {
          throw new Error(
      ...
      if (agent_config.memory) {
        if (agent_config.memory.memorySize) {
          if (
            agent_config.memory.memorySize >
            getGuardValue('agents.memory.memory_size_max')
          ) {
            throw new Error(
          ...
          if (
            agent_config.memory.memorySize <
            getGuardValue('agents.memory.memory_size_min')
          ) {
            throw new Error(
        ...
        if (agent_config.memory.shortTermMemorySize) {
          if (
            agent_config.memory.shortTermMemorySize >
            getGuardValue('agents.memory.short_term_memory_size_max')
          ) {
            throw new Error(
          ...
          if (
            agent_config.memory.shortTermMemorySize <
            getGuardValue('agents.memory.short_term_memory_size_min')
          ) {
            throw new Error(
      ...
      if (agent_config.plugins) {
        if (
          agent_config.plugins.length > getGuardValue('agents.plugins.max_size')
        ) {
          throw new Error(
        ...
        if (
          agent_config.plugins.length < getGuardValue('agents.plugins.min_size')
        ) {
          throw new Error(
      ...
      if (agent_config.mcpServers) {
        ...
        if (
          mcpServers.length > getGuardValue('agents.mcp_servers.max_servers')
        ) {
          throw new Error(
        ...
        if (
          mcpServers.length < getGuardValue('agents.mcp_servers.min_servers')
        ) {
          throw new Error(
        ...
        for (const serverName of mcpServers) {
          ...
          if (server.args) {
            if (
              server.args.length >
              getGuardValue('agents.mcp_servers.args.max_size')
            ) {
              throw new Error(
            ...
            if (
              server.args.length <
              getGuardValue('agents.mcp_servers.args.min_size')
            ) {
              throw new Error(
          ...
          if (server.env) {
            ...
            if (
              envKeys.length > getGuardValue('agents.mcp_servers.env.max_size')
            ) {
              throw new Error(
            ...
            if (
              envKeys.length < getGuardValue('agents.mcp_servers.env.min_size')
            ) {
              throw new Error(
            ...
            for (const [key, value] of Object.entries(server.env)) {
              if (
                key.length > getGuardValue('agents.mcp_servers.env.max_length')
              ) {
                throw new Error(
              ...
              if (
                key.length < getGuardValue('agents.mcp_servers.env.min_length')
              ) {
                throw new Error(
              ...
              if (value && typeof value === 'string') {
                if (
                  value.length >
                  getGuardValue('agents.mcp_servers.env.max_length')
                ) {
                  throw new Error(
                ...
                if (
                  value.length <
                  getGuardValue('agents.mcp_servers.env.min_length')
                ) {
                  throw new Error(

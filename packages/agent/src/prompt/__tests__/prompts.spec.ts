...
describe('prompts', () => {
  ...
  const createStepInfo = (stepNumber: number, stepName: string, status: string): StepInfo => ({
    stepNumber,
    stepName,
    status,
  });
  ...
  describe('rule constants', () => {
    it.each([
      ['interactiveRules', interactiveRules, 'INTERACTIVE MODE', ['Think step-by-step', 'ask the user specific questions']],
      ['autonomousRules', autonomousRules, 'AUTONOMOUS MODE', ['call tools in every response', 'NEXT STEPS:']],
      ['hybridRules', hybridRules, 'HYBRID MODE', ['WAITING_FOR_HUMAN_INPUT', 'FINAL ANSWER:']],
    ])('%s should contain correct content', (_, rules, modeHeader, keyPhrases) => {
  ...
  describe('modelSelectorSystemPrompt', () => {
    ...
    ])('should generate prompt %s', (_, nextStepsSection, shouldContainFocus) => {
  ...
  describe('modelSelectorRules', () => {
    ...
    ])('should generate rules %s', (_, nextStepsSection, analysisContent, shouldContainFocus) => {
  ...
  describe('finalAnswerRules', () => {
    it.each([
      ['complex answer', 'The analysis is complete. Results show positive trends.'],
  ...
  describe('agentSelectorPromptContent', () => {
    ...
    ])('should generate prompt content with %s', (_, agentInfo, input, expectedAgentCount) => {
  ...
  describe('planPrompt', () => {
    it.each([
      ['complex input', 'Analyze blockchain data and create a comprehensive report'],
      ['empty input', ''],
      ['special characters', 'Analyze data with special chars: @#$%^&*() and create report'],
    ...
    it('should include all required sections', () => {
      ...
      const expectedSections = [
        'RULES:', 'Maximum 5-7 steps total', 'FORMAT:', 'SOLUTION PLAN:',
        'Step 1: [Action name] - [Description of what to do]',
        'Checkpoints:', 'After step X: [What to verify]'
      ];
  ...
  describe('PromptPlanInteractive', () => {
    ...
    it('should generate interactive plan prompt with all main sections', () => {
      ...
      const result = PromptPlanInteractive(currentStep, stepHistory, defaultPlan);
    ...
    it.each([
      ...
      if (history.length > 0) {
        history.forEach(step => {
          expect(result).toContain(`STEP ${step.stepNumber}: ${step.stepName} (${step.status})`);
        });
      }
    });
  ...
  describe('Integration Tests', () => {
    it('should generate consistent prompts across all functions', () => {
      const agentConfig = createMockAgentConfig(createMockPrompt('Test content'));
      ...
      expect(modelSelectorSystemPrompt('test steps')).toContain('model selector');
      expect(planPrompt('test request')).toContain('Create a SIMPLE action plan');
      expect(agentSelectorPromptContent(agentMap, 'test')).toContain('Agent Router');

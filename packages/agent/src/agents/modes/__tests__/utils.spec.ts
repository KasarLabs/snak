...
import { Agent, ParsedPlan, ValidatorStepResponse, StepInfo } from '../types/index.js';
import {
...
const makeValidatorResponse = (overrides: Partial<ValidatorStepResponse> = {}): ValidatorStepResponse => ({
  steps: [
...
const makeMessage = (type: 'human' | 'ai' | 'tool', content: string, overrides: any = {}) => {
  switch (type) {
    case 'human':
    ...
    case 'ai':
      ...
      if (overrides.final) msg.additional_kwargs = { ...msg.additional_kwargs, final: overrides.final };
      return msg;
    case 'tool':
...
describe('Utils', () => {
  ...
  describe('formatParsedPlanSimple', () => {
    ...
    it('should handle empty plan', () => {
      const result = formatParsedPlanSimple({ summary: 'Empty plan', steps: [] });
  ...
  describe('createMaxIterationsResponse', () => {
    it('should create max iterations response with correct structure', () => {
      ...
      expect(message.content).toBe('Reaching maximum iterations for interactive agent. Ending workflow.');
  ...
  describe('filterMessagesByShortTermMemory', () => {
    ...
    it('should prioritize iteration agent messages', () => {
      ...
      const result = filterMessagesByShortTermMemory(messagesWithIterationAgents, 3);
    ...
    ])('should handle edge case: $messages.length messages, limit $limit', ({ messages, limit, expected }) => {
  ...
  describe('isTerminalMessage', () => {
    it.each([
      ...
      { content: 'This is the FINAL ANSWER to your question', final: false, expected: true },
      { content: 'The PLAN_COMPLETED successfully', final: false, expected: true },
      { content: 'This is a regular message', final: false, expected: false },
    ])('should return $expected for message with content "$content" and final=$final', ({ content, final, expected }) => {
  ...
  describe('isTokenLimitError', () => {
    ...
    ])('should return $expected for error message "$message"', ({ message, expected }) => {

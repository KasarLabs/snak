      ...
      withStructuredOutput() {
        return {
          invoke: jest.fn(async () => ({
            steps: [{ stepNumber: 1, stepName: 'Step 1', description: 'desc', status: 'pending', type: 'message', result: '' }],
            summary: 'sum',
          })),
        };
      },
...
jest.mock('@langchain/core/prompts', () => ({
  ...
  ChatPromptTemplate: class {
    static fromMessages() { return new (class MockChatPromptTemplate {
...
const makeState = (over: Partial<any> = {}) => ({
  ...
  rag: '',
  plan: {
    steps: [{ stepNumber: 1, stepName: 'S1', description: 'do something', status: 'pending', type: 'message', result: '' }],
    summary: '',
  },
  currentStepIndex: 0,
  retry: 0,
...
describe('AutonomousAgent', () => {
  ...
  describe('System prompt building', () => {
    it.each([
      ...
      ['empty tools', [], ''],
    ])('buildSystemPrompt %s includes correct content', async (_, tools, expectedTools) => {
      await agent.initialize();
      (agent as any).toolsList = tools;
      
      const prompt = (agent as any).buildSystemPrompt(makeState(), makeConfig());
  ...
  describe('Routing logic', () => {
    it.each([
      ...
      ['EXEC_VALIDATOR final', Agent.EXEC_VALIDATOR, { final: true }, 'adaptive_planner'],
    ...
    ])('shouldContinueAutonomous %s -> %s', (_, messageProps, expectedRoute, graphStep = 0, configOverrides = { max_graph_steps: 100 }, lastAgent = Agent.EXECUTOR) => {
      const state = makeState({
        last_agent: lastAgent,
        last_message: new AIMessageChunk({ content: 'test', ...messageProps }),
        currentGraphStep: graphStep,
      });
      
      const next = (agent as any).shouldContinueAutonomous(state, { configurable: configOverrides });
      expect(next).toBe(expectedRoute);
    });

    describe('shouldContinueHybrid', () => {
      beforeEach(() => {
        (agent as any).agentConfig = { ...snak.getAgentConfig(), mode: AgentMode.HYBRID };
      });
      ...
      it.each([
        ...
        const state = makeState({
          ...
          last_message: new AIMessageChunk({ content: 'test', ...messageProps }),
          plan: { steps: [{ ...makeState().plan.steps[0], type: stepType }], summary: '' },
  ...
  describe('Planner operations', () => {
    ...
    it.each([
      ...
      ['hybrid mode', Agent.PLANNER, { steps: [], summary: '' }, AgentMode.HYBRID],
    ])('%s creates plan successfully', async (_, lastAgent, plan, mode = AgentMode.AUTONOMOUS) => {
  ...
  describe('Summarization', () => {
    ...
    ])('summarizeMessages %s returns correct result', async (_, messageCount, expectedResult) => {
      const msgs: any[] = [];
      for (let i = 0; i < messageCount; i++) {
        msgs.push(new AIMessageChunk({ 
          content: `exec-${i}`, 
    ...
    it('summarizeMessages handles model not found error', async () => {
      ...
      await expect((agent as any).summarizeMessages(state)).rejects.toThrow('Model not found in ModelSelector');
    });

    it('summarizeMessages stops once token budget exceeds 11000', async () => {
      const msgs: any[] = [new HumanMessage('start')];
      for (let i = 0; i < 120; i++) {
        const m = new AIMessageChunk({ content: `exec-${i}`, additional_kwargs: { from: Agent.EXECUTOR } });
        m.response_metadata = { usage: { completion_tokens: 100 } };
        msgs.push(m);
      }
      const res = await (agent as any).summarizeMessages(makeState({ messages: msgs }));
      
      expect(res.messages).toBeInstanceOf(Array);
      expect(res.messages!.length).toBeLessThan(msgs.length);
      
      const hasStartMessage = res.messages!.some((msg: any) => 
        msg instanceof HumanMessage && msg.content === 'start'
      );
      expect(hasStartMessage).toBe(true);
      
      const hasSummaryMessage = res.messages!.some((msg: any) => 
        msg.additional_kwargs?.from === Agent.SUMMARIZE
      );
  ...
  describe('Configuration and compilation', () => {
    ...
    it('buildWorkflow throws error when memoryAgent is not setup', () => {
      ...
      expect(() => (agent as any).buildWorkflow()).toThrow('MemoryAgent is not setup');
    });
  });
  ...
  describe('Edge cases and limits', () => {
    ...
    ])('shouldContinueAutonomous %s routes correctly', (_, configOverrides, graphStep, expectedRoute) => {
      const state = makeState({ 
        last_agent: Agent.TOOLS, 
        currentGraphStep: graphStep,
        last_message: new ToolMessage('toolA', 'tool result', 'test-tool-call')
      });
    ...
    it('workflow with tool usage: plan → execute → tools → validate', async () => {
      ...
      const mockModelWithToolCalls = {
        withStructuredOutput() {
          return {
            invoke: jest.fn(async () => ({
              steps: [{ stepNumber: 1, stepName: 'Test Step', description: 'Use testTool', status: 'pending', type: 'tools', result: '' }],
              summary: 'Test plan',
            })),
          };
        },
      ...
      const mockSelector = {
        getModels() { return { fast: mockModelWithToolCalls }; },
        async selectModelForMessages() { return { model_name: 'fast', model: mockModelWithToolCalls }; },
      };
      ...
      const executionState = await (agent as any).callModel(enrichedPlanState, makeConfig());
      ...
      expect(executionState.last_message.tool_calls[0].args).toEqual({ param1: 'value1', param2: 'value2' });
      ...
      expect(mockTool.execute).toHaveBeenCalledWith({ param1: 'value1', param2: 'value2' });
      ...
      expect(toolResult.messages[0].content).toContain('Tool executed with args');
  ...
  describe('Failure scenario tests', () => {
    it.each([
      ['plan validation failure', 'Plan validation failed: Plan is too complex', false, false],
      ['plan validation error', 'Failed to validate plan: Model error occurred', true, false],
      ['plan execution failure', 'Model invocation failed: Network error', true, false],
      ['tool execution failure', 'Tool execution failed: Invalid parameters', true, false],
      ['max iterations reached', 'max-iter-100', false, false],
      ['validation routing error', 'Routing logic error: Invalid state', false, false],
    ])('handles %s correctly', async (_, errorMessage, hasError, isValidated) => {
      await agent.initialize();
      ...
      if (errorMessage.includes('max-iter')) {
        ...
        const mockCallModel = jest.fn(async (state: any) => ({
          messages: { content: errorMessage, additional_kwargs: { error: true, tool_error: 'Invalid parameters' } },
          last_message: { content: errorMessage, additional_kwargs: { error: true, tool_error: 'Invalid parameters' } },
          last_agent: Agent.EXECUTOR,
        ...
        const state = makeState();
        await expect((agent as any).planExecution(state, makeConfig()))
        ...
        const mockValidatorPlanner = jest.fn(async (state: any) => ({
          last_message: {
            ...
            additional_kwargs: { error: hasError, validated: isValidated, from: 'planner_validator' },
          },
        ...
        expect(res.last_message.additional_kwargs.validated).toBe(isValidated);
      }
    });
    ...
    it('handles complete failure workflow: plan → validation error → end', async () => {
      ...
      const mockPlanExecution = jest.fn(async (state: any) => ({
        last_message: { content: 'Plan created successfully', additional_kwargs: {} },
        last_agent: Agent.PLANNER,
        plan: { steps: [{ stepNumber: 1, stepName: 'Test Step', description: 'Test description', status: 'pending', type: 'message', result: '' }], summary: 'Test plan' },
        currentStepIndex: 0,
        currentGraphStep: state.currentGraphStep + 1,
      }));
      ...
      const mockValidatorPlanner = jest.fn(async (state: any) => ({
        last_message: {
          ...
          additional_kwargs: { error: true, validated: false, from: 'planner_validator' },
        },
        last_agent: Agent.PLANNER_VALIDATOR,
        currentStepIndex: 0,
        retry: 0,
      ...
      expect(validationState.last_message.additional_kwargs.validated).toBe(false);
      
      const mockHandleValidatorRouting = jest.fn((state: any) => {
        if (state.last_agent === Agent.PLANNER_VALIDATOR && state.last_message.additional_kwargs.error === true) {
          return 'end';
        }
        return 'executor';
      });

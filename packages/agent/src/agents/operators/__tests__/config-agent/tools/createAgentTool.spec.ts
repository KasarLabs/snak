...
describe('createAgentTool', () => {
  ...
  const setupMocks = () => {
    ...
    mockNormalize = require('../../../config-agent/tools/normalizeAgentValues.js').normalizeNumericValues;
    
    mockPostgres.Query.mockImplementation((query: string, params: any[]) => ({ query, params }));
    mockNormalize.mockReturnValue({
      normalizedConfig: { interval: 5, max_iterations: 15, memory: { shortTermMemorySize: 5, memorySize: 20 } },
      appliedDefaults: []
    });
  };

  const mkRow = (overrides = {}) => ({ id: 1, name: 'test-agent', ...overrides });
  ...
  describe('tool configuration', () => {
    ...
    it('should validate schema with safeParse', () => {
      ...
      const result = schema.safeParse({ name: 'test', group: 'test', description: 'test' });
  ...
  describe('normalization & INSERT parameters', () => {
    it.each([
      {
        ...
        expected: ['test', 'test', 'test', '', '', '', null, 5, null, false, 5, 20, false, null, 'interactive', 15]
      },
      {
        name: 'all fields present',
        input: {
          name: 'full', group: 'trading', description: 'desc',
          lore: ['l1'], objectives: ['o1'], knowledge: ['k1'],
          system_prompt: 'prompt', interval: 300, plugins: ['p1'],
          memory: { enabled: true, shortTermMemorySize: 50, memorySize: 100 },
          rag: { enabled: true, embeddingModel: 'model' },
          mode: 'autonomous', max_iterations: 10
        },
        expected: ['full', 'trading', 'desc', ['l1'], ['o1'], ['k1'], 'prompt', 5, ['p1'], true, 5, 20, true, 'model', 'autonomous', 15]
      },
      {
        name: 'empty arrays',
        input: { name: 'empty', group: 'test', description: 'test', lore: [], objectives: [], knowledge: [], plugins: [] },
        expected: ['empty', 'test', 'test', [], [], [], null, 5, [], false, 5, 20, false, null, 'interactive', 15]
      }
    ...
    it('should use normalized values from normalizeNumericValues', async () => {
      mockNormalize.mockReturnValue({
        normalizedConfig: { interval: 10, max_iterations: 25, memory: { shortTermMemorySize: 15, memorySize: 50 } },
        appliedDefaults: ['interval set to 10', 'max_iterations set to 25']
      });
      mockPostgres.query.mockResolvedValue([mkRow()]);
      
      await createAgentTool.func({ name: 'test', group: 'test', description: 'test' });
      
      expect(mockNormalize).toHaveBeenCalledWith({ name: 'test', group: 'test', description: 'test' });
      expect(mockPostgres.Query).toHaveBeenCalledWith(
        expect.any(String),
    ...
    it('should handle null/undefined values with defaults', async () => {
      ...
      await createAgentTool.func({
        name: 'test', group: 'test', description: 'test',
        lore: null, objectives: undefined, knowledge: null,
        system_prompt: null, plugins: undefined, memory: null, rag: null
      });
      
      expect(mockPostgres.Query).toHaveBeenCalledWith(
        expect.any(String),
        expect.arrayContaining(['', '', '', null, null, false, 5, 20, false, null])
      );
    });
  });
  ...
  describe('success cases', () => {
    it('should return success with data when agent created', async () => {
      ...
      const result = await createAgentTool.func({ name: 'success-agent', group: 'test', description: 'test' });
    ...
    it('should include appliedDefaults in message when present', async () => {
      ...
      const result = await createAgentTool.func({ name: 'test', group: 'test', description: 'test' });
      const parsed = asJson(result);
      
      expect(parsed.message).toContain('Note: interval set to 5; max_iterations set to 15');
    });
  });
  ...
  describe('failure cases', () => {
    ...
    it.each([
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error creating agent:'));
      expect(parsed.success).toBe(false);
  ...
  describe('pass-through validation', () => {
    it('should preserve complex objects/arrays as-is', async () => {
      const complexInput = {
        name: 'complex', group: 'test', description: 'test',
        ...
        rag: { enabled: true, embeddingModel: 'very-long-model-name-with-special-chars' }
      };
      mockPostgres.query.mockResolvedValue([mkRow()]);
      ...
      expect(mockPostgres.Query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          complexInput.lore, complexInput.objectives, complexInput.knowledge,
          complexInput.plugins, complexInput.memory.enabled, complexInput.rag.enabled, complexInput.rag.embeddingModel
        ])
      );
    });
  });

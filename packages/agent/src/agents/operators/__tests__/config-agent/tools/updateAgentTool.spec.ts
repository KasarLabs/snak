...
describe('updateAgentTool', () => {
  ...
  describe('search behavior', () => {
    it.each([
      ...
      ['name', 'test-agent', 'name', 'test-agent', 'SELECT * FROM agents WHERE name = $1'],
      [undefined, 'test-agent', 'name', 'test-agent', 'SELECT * FROM agents WHERE name = $1'],
      [null, 'test-agent', 'name', 'test-agent', 'SELECT * FROM agents WHERE name = $1'],
      ['invalid', 'test-agent', 'name', 'test-agent', 'SELECT * FROM agents WHERE name = $1'],
    ])('should search by %s when searchBy=%s', async (searchBy, identifier, expectedSearchBy, expectedValue, expectedQuery) => {
      const mockAgent = createAgent({ name: identifier });
      const mockUpdatedAgent = { ...mockAgent, description: 'Updated' };

      mockNormalize.mockReturnValue({ normalizedConfig: { description: 'Updated' }, appliedDefaults: [] });
      mockPostgres.query
        .mockResolvedValueOnce([mockAgent])
        .mockResolvedValueOnce([mockUpdatedAgent]);

      const result = await updateAgentTool.func({ identifier, searchBy: searchBy as any, updates: { description: 'Updated' } });

      expect(mockPostgres.query).toHaveBeenNthCalledWith(1, expect.objectContaining({
        query: expectedQuery,
  ...
  describe('update execution', () => {
    it('should handle multiple fields update', async () => {
      const updates = { name: 'new-name', description: 'Updated', group: 'trading' };
      const mockAgent = createAgent();
      const mockUpdatedAgent = { ...mockAgent, ...updates };

      mockNormalize.mockReturnValue({ normalizedConfig: updates, appliedDefaults: [] });
      mockPostgres.query
        .mockResolvedValueOnce([mockAgent])
        .mockResolvedValueOnce([mockUpdatedAgent]);
    ...
    it('should handle no valid fields to update', async () => {
      const mockAgent = createAgent();
      mockNormalize.mockReturnValue({ normalizedConfig: {}, appliedDefaults: [] });
  ...
  describe('error handling', () => {
    it('should handle Postgres.Query constructor errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error updating agent:'));

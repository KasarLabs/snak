...
describe('updateAgentTool', () => {
  ...
  describe('tool configuration', () => {
    it('should have correct name and description', () => {
      ...
      expect(updateAgentTool.description).toContain('Update/modify/change/rename specific properties');
    });
  ...
  describe('function execution', () => {
    it('should update agent by name with single field', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1 WHERE name = $2 RETURNING *', ['Updated description', 'test-agent']);
      
      const parsedResult = JSON.parse(result);
      expect(parsedResult.success).toBe(true);
      expect(parsedResult.message).toBe('Agent "test-agent" updated successfully');
    ...
    it('should update agent by ID with multiple fields', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "name" = $1, "description" = $2, "group" = $3, "mode" = $4, "max_iterations" = $5 WHERE id = $6 RETURNING *', ['updated-agent', 'Updated description', 'rpc', 'interactive', 15, 1]);
      
      const parsedResult = JSON.parse(result);
      expect(parsedResult.success).toBe(true);
      expect(parsedResult.message).toBe('Agent "old-agent" updated successfully');
    });
    ...
    it('should update agent with array fields', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['array-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "lore" = $1, "objectives" = $2, "knowledge" = $3, "plugins" = $4 WHERE name = $5 RETURNING *', [
    ...
    it('should update agent with complex memory configuration', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['memory-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "memory" = $1 WHERE name = $2 RETURNING *', [
    ...
    it('should update agent with complex RAG configuration', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['rag-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "rag" = $1 WHERE name = $2 RETURNING *', [
        { enabled: true, topK: 10, embeddingModel: 'Xenova/all-MiniLM-L6-v2' },
        'rag-agent'
      ]);
    ...
    it('should update agent with all fields', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['full-update-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "name" = $1, "description" = $2, "group" = $3, "lore" = $4, "objectives" = $5, "knowledge" = $6, "system_prompt" = $7, "interval" = $8, "plugins" = $9, "memory" = $10, "rag" = $11, "mode" = $12, "max_iterations" = $13 WHERE name = $14 RETURNING *', [
        'full-update-agent',
    ...
    it('should handle empty arrays for array fields', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['empty-arrays-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "lore" = $1, "objectives" = $2, "knowledge" = $3, "plugins" = $4 WHERE name = $5 RETURNING *', [[], [], [], [], 'empty-arrays-agent']);
  ...
  describe('error handling', () => {
    ...
    it('should handle database query errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Error updating agent: Error: Database connection failed');
    ...
    it('should handle database query errors with non-Error objects', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Error updating agent: String error');
    ...
    it('should handle Postgres.Query constructor errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Error updating agent: Error: Invalid query syntax');
  ...
  describe('edge cases', () => {
    it('should handle agent name with special characters', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1 WHERE name = $2 RETURNING *', ['Updated description', 'agent-with-dashes_and_underscores']);
    ...
    it('should handle very long agent names', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1 WHERE name = $2 RETURNING *', ['Updated description', longName]);
    ...
    it('should handle very long descriptions', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['long-desc-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1 WHERE name = $2 RETURNING *', [longDescription, 'long-desc-agent']);
    ...
    it('should handle large arrays in array fields', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['large-arrays-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "lore" = $1, "objectives" = $2, "knowledge" = $3, "plugins" = $4 WHERE name = $5 RETURNING *', [largeArray, largeArray, largeArray, largeArray, 'large-arrays-agent']);
    ...
    it('should handle complex nested objects in memory and rag', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['complex-objects-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "memory" = $1, "rag" = $2 WHERE name = $3 RETURNING *', [complexMemory, complexRag, 'complex-objects-agent']);
    ...
    it('should handle empty string values', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['empty-strings-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1, "group" = $2, "system_prompt" = $3, "mode" = $4 WHERE name = $5 RETURNING *', ['', '', '', '', 'empty-strings-agent']);
    ...
    it('should normalize zero and negative numeric values to defaults', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['numeric-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "interval" = $1, "max_iterations" = $2 WHERE name = $3 RETURNING *', [5, 15, 'numeric-agent']);
      
      const parsedResult = JSON.parse(result);
      expect(parsedResult.success).toBe(true);
      expect(parsedResult.message).toContain('interval set to default value (5)');
      expect(parsedResult.message).toContain('max_iterations set to default value (15)');
    });
    ...
    it('should handle very large numeric values', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['large-numeric-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "interval" = $1, "max_iterations" = $2 WHERE name = $3 RETURNING *', [999999, 999999, 'large-numeric-agent']);
  ...
  describe('parameter validation', () => {
    it('should handle numeric string ID', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "description" = $1 WHERE id = $2 RETURNING *', ['Updated description', 123]);
  ...
  describe('numeric value normalization', () => {
    it('should normalize negative max_iterations values to default', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "max_iterations" = $1 WHERE name = $2 RETURNING *', [15, 'test-agent']);
    ...
    it('should accept positive max_iterations values', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "max_iterations" = $1 WHERE name = $2 RETURNING *', [25, 'test-agent']);
    ...
    it('should normalize negative interval values to default', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "interval" = $1 WHERE name = $2 RETURNING *', [5, 'test-agent']);
    ...
    it('should accept positive interval values', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "interval" = $1 WHERE name = $2 RETURNING *', [2000, 'test-agent']);
    ...
    it('should normalize negative memory configuration values to defaults', async () => {
      ...
      const parsedResult = JSON.parse(result);
      expect(parsedResult.success).toBe(true);
      expect(parsedResult.message).toContain('memory.shortTermMemorySize set to default value (5)');
      expect(parsedResult.message).toContain('memory.memorySize set to default value (20)');
    });
    ...
    it('should normalize negative RAG configuration values to default', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "rag" = $1 WHERE name = $2 RETURNING *', [
        { enabled: true, topK: 10 },
        'test-agent'
      ]);
      
      const parsedResult = JSON.parse(result);
      expect(parsedResult.success).toBe(true);
      expect(parsedResult.message).toContain('rag.topK set to default value (10)');
    });
    ...
    it('should normalize multiple invalid numeric values in single update', async () => {
      ...
      expect(mockPostgres.Query).toHaveBeenCalledTimes(2);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(1, 'SELECT * FROM agents WHERE name = $1', ['test-agent']);
      expect(mockPostgres.Query).toHaveBeenNthCalledWith(2, 'UPDATE agents SET "interval" = $1, "max_iterations" = $2, "memory" = $3 WHERE name = $4 RETURNING *', [
        5, // Default interval

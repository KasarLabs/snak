...
export class MCPAgentService {
  ...
  constructor(config: MCPAgentServiceConfig = {}) {
    ...
    if (this.debug) {
      logger.debug(
        `MCPAgent initialized with ${this.tools.length} tools: ${this.tools
          .map((t) => t.name)
          .join(', ')}`
      );
    }
  ...
  public async init(): Promise<void> {
    try {
      ...
      logger.debug('MCPAgent initialized with React agent');
    } catch (error) {
  ...
  public async dispose(): Promise<void> {
    logger.debug('MCPAgentService disposed');
  }
}